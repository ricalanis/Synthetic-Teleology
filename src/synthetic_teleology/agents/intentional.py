"""Intentional State agent for the Synthetic Teleology framework.

Implements intentional state mapping (beliefs, desires, intentions) onto the
framework's teleological loop:
...
currently executing policy).  The deliberation cycle is:
...
IntentionalStateAgent
    Full intentional-state agent extending :class:`BaseAgent` with belief,
...
class IntentionalStateAgent(BaseAgent):
    """Intentional-state agent mapped onto the Synthetic Teleology loop.
    ...
    * **Intentions** -- the currently committed plan (a ``PolicySpec``
    ...
    The agent delegates evaluation, goal revision, and planning to
    ...
    def __init__(
        ...
        # Core: Beliefs, Desires, Intentions
    ...
    # -- intentional-state-specific methods ---------------------------------
    ...
    def add_desire(self, goal: Goal, priority: float = 0.5) -> None:
        ...
        logger.debug(
            "IntentionalStateAgent %s: added desire %s (priority=%.3f, total_desires=%d)",
            self.id,
            goal.goal_id,
            priority,
            len(self._desires),
    ...
    def remove_desire(self, goal_id: str) -> bool:
        ...
        if removed:
            logger.debug(
                "IntentionalStateAgent %s: removed desire %s (remaining=%d)",
                self.id,
                goal_id,
                len(self._desires),
            )
    ...
    def reconsider(self, state: StateSnapshot) -> Goal | None:
        ...
        This is the "filter" function that determines whether the
        ...
        for base_priority, goal in self._desires:
            ...
            try:
                ...
                logger.debug(
                    "IntentionalStateAgent %s: cannot evaluate desire %s: %s",
                    self.id,
                    goal.goal_id,
                    exc,
                )
        ...
        if top_goal.goal_id != current_goal.goal_id:
            logger.info(
                "IntentionalStateAgent %s: reconsidering desires: switching from %s to %s "
                "(priority=%.3f)",
                self.id,
    ...
    def clear_intentions(self) -> None:
        ...
        logger.debug("IntentionalStateAgent %s: intentions cleared", self.id)
    ...
    def revise_goal(
        ...
        if proposed_goal is not None and proposed_goal.objective != goal.objective:
            ...
            new_goal, revision = goal.revise(
                ...
                reason="intentional_objective_update",
                eval_signal=delta,
            )
            ...
            logger.info(
                "IntentionalStateAgent %s: revised goal objective %s -> %s (score=%.3f)",
                self.id,
                goal.goal_id,
                new_goal.goal_id,
                delta.score,
        ...
        # Phase 2: Desire reconsideration -- should we switch desires entirely?
        ...
        if switched_desire is not None:
            ...
            if switched_desire.goal_id != active.goal_id:
                ...
                switch_revision = GoalRevision(
                    ...
                    reason="intentional_desire_switch",
                    eval_signal=delta,
                )
                ...
                logger.info(
                    "IntentionalStateAgent %s: switched active desire to %s",
                    self.id,
    ...
    def plan(self, goal: Goal, state: StateSnapshot) -> PolicySpec:
        ...
        if (
            ...
            logger.debug(
                "IntentionalStateAgent %s: reusing existing intention "
        ...
        logger.debug(
            "IntentionalStateAgent %s: new intention formed with %d actions (%.1f ms)",
            self.id,
            policy.size,
            elapsed_ms,
        )
    ...
    def select_action(self, policy: PolicySpec) -> ActionSpec:
        ...
        if not policy.actions:
            logger.warning("IntentionalStateAgent %s: empty intention, selecting noop", self.id)
        ...
        if policy.is_stochastic and policy.probabilities:
            ...
            if self._intention_index < len(policy.actions):
                ...
                # Intention exhausted
                logger.debug(
                    "IntentionalStateAgent %s: intention exhausted, selecting last action",
                    self.id,
                )
    ...
    def on_goal_revised(self, revision: GoalRevision) -> None:
        ...
        logger.info(
            "IntentionalStateAgent %s: goal revised (%s -> %s), reason: %s",
            self.id,
    ...
    def on_constraint_violated(self, violation: str) -> None:
        ...
        logger.warning(
            "IntentionalStateAgent %s: constraint violated: %s",
            self.id,
            violation,
        )
    ...
    def on_reflection_triggered(self) -> None:
        ...
        logger.debug(
            "IntentionalStateAgent %s: reflection at step %d (score=%.3f, "
            "desires=%d, beliefs=%d)",
            self.id,
            self.step_count,
            self._last_eval.score if self._last_eval else 0.0,
